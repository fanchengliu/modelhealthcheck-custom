...
模型中转状态检测是一个用于监控 AI 模型 API 可用性与延迟的健康面板。当前仓库是一个面向自托管与后台管理场景的衍生版本：在保留 Dashboard、分组视图、状态 API 与基础健康检查能力的同时，这个版本已经明显偏向“可自托管、可后台管理、可多存储后端运行”的实现方向。
...
- [`Arron196/modelhealthcheck`](https://github.com/Arron196/modelhealthcheck)
- [`BingZi-233/check-cx`](https://github.com/BingZi-233/check-cx)

当前仓库在其基础上结合自用场景进行了较多定制与扩展，包括但不限于：

- 后台管理控制面
- 分组管理与删除逻辑调整
- Dashboard 刷新 / 缓存链路修复
- 通知中心与 Telegram 集成
- 多存储后端支持（SQLite / PostgreSQL / Supabase）
- 部署与运行时适配

本仓库为非官方衍生版本，后续功能、运维方式与发布内容均由当前仓库独立维护，与原项目作者无直接关联。

## 与上游代码的关系

- **参考来源**：当前实现同时参考了 `Arron196/modelhealthcheck` 与更早期的 `BingZi-233/check-cx`。
- **维护关系**：当前仓库不是上述任一项目的官方分支、镜像或协作仓库；后续提交、功能取舍和部署方式均由当前仓库自行决定。
- **文档目的**：下面的“新增功能 / 去掉或调整的功能”用于帮助使用者快速理解当前仓库相对参考项目的定位与差异。
...
This repository is a non-official derivative maintained primarily for personal use, ongoing custom development, archival, and reference purposes. It is published publicly for transparency and continuity, but no general support, compatibility guarantees, or stable interfaces are promised.

If any content in this repository is considered infringing, inappropriate, or should not be distributed, please contact the maintainer and the relevant content will be reviewed and removed or handled as appropriate.

## Tooling Acknowledgements


Development, debugging, documentation cleanup, and release preparation for this repository were also assisted by OpenClaw and Codex as tooling/workflow support. Final review, selection, and publishing decisions remain with the current maintainer.
