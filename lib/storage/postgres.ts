...
import {
  ...
  getDefaultTelegramNotificationSettingsRow,
  mapAdminUserRecord,
  ...
  mapSiteSettingsRow,
  mapTelegramNotificationSettingsRow,
  nowIso,
...
import type {
  ...
  TelegramNotificationSettingsMutationInput,
  StorageCapabilities,
} from "./types";
  ...
  return {
        ...
        try {
          if (input.id) {
            const updateResult = await pool.query(
              `
                ...
                WHERE id = $1
              `,
              [input.id, input.group_name, input.website_url ?? null, input.tags ?? "", timestamp]
            );
          ...
          await pool.query(
            `
              ...
              VALUES ($1, $2, $3, $4, $5, $6)
            `,
            [payloadId, input.group_name, input.website_url ?? null, input.tags ?? "", timestamp, timestamp]
          );
        } catch (error) {
    ...
    telegramNotifications: {
      async getSingleton(singletonKey) {
        await ensureReady();
        try {
          const result = await pool.query(`SELECT * FROM telegram_notification_settings WHERE singleton_key = $1 LIMIT 1`, [singletonKey]);
          return result.rows[0] ? mapTelegramNotificationSettingsRow(mapRows(result.rows)[0]) : null;
        } catch (error) {
          wrapError("读取 Telegram 通知设置", error);
        }
      },
      async upsert(input: TelegramNotificationSettingsMutationInput) {
        await ensureReady();
        try {
          await pool.query(`
            INSERT INTO telegram_notification_settings (singleton_key, enabled, bot_token, chat_id, notify_on_group_test, notify_on_full_test, notify_on_partial_refresh, notify_on_auto_refresh, only_on_failure_for_auto_refresh)
            VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
            ON CONFLICT (singleton_key) DO UPDATE SET
              enabled = excluded.enabled,
              bot_token = excluded.bot_token,
              chat_id = excluded.chat_id,
              notify_on_group_test = excluded.notify_on_group_test,
              notify_on_full_test = excluded.notify_on_full_test,
              notify_on_partial_refresh = excluded.notify_on_partial_refresh,
              notify_on_auto_refresh = excluded.notify_on_auto_refresh,
              only_on_failure_for_auto_refresh = excluded.only_on_failure_for_auto_refresh,
              updated_at = timezone('utc', now())
          `, [input.singleton_key, input.enabled, input.bot_token, input.chat_id, input.notify_on_group_test, input.notify_on_full_test, input.notify_on_partial_refresh, input.notify_on_auto_refresh, input.only_on_failure_for_auto_refresh]);
        } catch (error) {
          wrapError("保存 Telegram 通知设置", error);
        }
      },
    },
    notifications: {
