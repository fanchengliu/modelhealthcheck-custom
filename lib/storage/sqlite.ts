  ...
  return {
    ...
    telegramNotifications: {
      async getSingleton(singletonKey) {
        ...
        try {
          const row = db
            .prepare(`SELECT * FROM telegram_notification_settings WHERE singleton_key = ? LIMIT 1`)
            .get(singletonKey) as Record<string, unknown> | undefined;
          return row ? mapTelegramNotificationSettingsRow(row) : null;
        } catch (error) {
        ...
        try {
          db.prepare(`
            INSERT INTO telegram_notification_settings (
              singleton_key, enabled, bot_token, chat_id, notify_on_group_test, notify_on_full_test, notify_on_partial_refresh, notify_on_auto_refresh, only_on_failure_for_auto_refresh, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(singleton_key) DO UPDATE SET
              enabled = excluded.enabled,
          ...
          `).run(
            ...
            input.bot_token,
            input.chat_id,
            input.notify_on_group_test ? 1 : 0,
            input.notify_on_full_test ? 1 : 0,
            input.notify_on_partial_refresh ? 1 : 0,
            input.notify_on_auto_refresh ? 1 : 0,
            input.only_on_failure_for_auto_refresh ? 1 : 0,
            now,
            now
          );
        } catch (error) {
