...
import type {
  ...
  TelegramNotificationSettingsRow,
...
export interface TelegramNotificationSettingsMutationInput {
  singleton_key: string;
  enabled: boolean;
  bot_token: string;
  chat_id: string;
  notify_on_group_test: boolean;
  notify_on_full_test: boolean;
  notify_on_partial_refresh: boolean;
  notify_on_auto_refresh: boolean;
  only_on_failure_for_auto_refresh: boolean;
}
...
export interface ControlPlaneStorage {
  ...
  telegramNotifications: {
    getSingleton(singletonKey: string): Promise<TelegramNotificationSettingsRow | null>;
    upsert(input: TelegramNotificationSettingsMutationInput): Promise<void>;
  };
}
