...
export const POSTGRES_CONTROL_PLANE_SCHEMA_STATEMENTS = [
  ...
  `
    CREATE TABLE IF NOT EXISTS telegram_notification_settings (
      ...
      notify_on_group_test boolean NOT NULL DEFAULT true,
      notify_on_full_test boolean NOT NULL DEFAULT true,
      notify_on_partial_refresh boolean NOT NULL DEFAULT false,
      notify_on_auto_refresh boolean NOT NULL DEFAULT false,
      only_on_failure_for_auto_refresh boolean NOT NULL DEFAULT true,
      created_at timestamptz NOT NULL DEFAULT timezone('utc', now()),
      updated_at timestamptz NOT NULL DEFAULT timezone('utc', now())
    )
  `,
...
export const SQLITE_CONTROL_PLANE_SCHEMA_STATEMENTS = [
  ...
  `
    CREATE TABLE IF NOT EXISTS telegram_notification_settings (
      ...
      bot_token text NOT NULL DEFAULT '',
      chat_id text NOT NULL DEFAULT '',
      notify_on_group_test integer NOT NULL DEFAULT 1,
      notify_on_full_test integer NOT NULL DEFAULT 1,
      notify_on_partial_refresh integer NOT NULL DEFAULT 0,
      notify_on_auto_refresh integer NOT NULL DEFAULT 0,
      only_on_failure_for_auto_refresh integer NOT NULL DEFAULT 1,
      created_at text NOT NULL,
      updated_at text NOT NULL
    )
  `,
...
export function getDefaultTelegramNotificationSettingsRow(): TelegramNotificationSettingsRow {
  return {
...
export function mapTelegramNotificationSettingsRow(row: LooseRow): TelegramNotificationSettingsRow {
  return {
    singleton_key: toRequiredString(row.singleton_key) || "global",
    enabled: toBoolean(row.enabled),
    bot_token: toRequiredString(row.bot_token),
    chat_id: toRequiredString(row.chat_id),
    notify_on_group_test: toBoolean(row.notify_on_group_test),
    notify_on_full_test: toBoolean(row.notify_on_full_test),
    notify_on_partial_refresh: toBoolean(row.notify_on_partial_refresh),
    notify_on_auto_refresh: toBoolean(row.notify_on_auto_refresh),
    only_on_failure_for_auto_refresh: toBoolean(row.only_on_failure_for_auto_refresh),
    created_at: toOptionalString(row.created_at) ?? undefined,
    updated_at: toOptionalString(row.updated_at) ?? undefined,
  };
}
