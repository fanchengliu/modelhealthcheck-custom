    ...
    telegramNotifications: {
      async getSingleton(singletonKey) {
        ...
        const {data, error} = await client
          ...
          .maybeSingle();
        if (error) {
          wrapStorageError("读取 Telegram 通知设置", error);
        }
        return data ? mapTelegramNotificationSettingsRow(data as Record<string, unknown>) : null;
      },
      async upsert(input: TelegramNotificationSettingsMutationInput) {
        await ensureReady();
        const client = createAdminClient({allowDraft});
        const {error} = await client.from("telegram_notification_settings").upsert(input);
        if (error) {
          wrapStorageError("保存 Telegram 通知设置", error);
        }
      },
    },
    notifications: {
