...
export async function fetchWithCache(
  ...
  const { trendPeriod, forceFresh, bypassCache, onBackgroundUpdate, revalidateIfFresh } = options;
  ...
  // 仅绕过缓存：不触发 provider 实测，只要求服务端返回最新聚合结果
  if (bypassCache) {
    const { data, etag } = await fetchFromNetwork(trendPeriod, cached?.etag, false, true);
    if (data) {
      recordMiss(false);
      setCache(trendPeriod, data, etag);
      return { data, fromCache: false, isRevalidating: false };
