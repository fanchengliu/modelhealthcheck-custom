...
import {ensureCheckPoller, runChecksForConfigs} from "./poller";
...
export function invalidateGroupDashboardCache(): void {
...
export async function loadGroupDashboardData(
  ...
  options?: { refreshMode?: RefreshMode; trendPeriod?: AvailabilityPeriod; bypassCache?: boolean; forceRefreshNow?: boolean; forceRefreshConfig?: boolean }
): Promise<GroupDashboardData | null> {
  ensureOfficialStatusPoller();
  ensureCheckPoller();

  const allConfigs = await loadProviderConfigsFromDB({forceRefresh: options?.forceRefreshConfig === true});
  ...
  console.log(`[check-cx][group-data] target=${targetGroupName} totalMatched=${groupConfigs.length} active=${groupConfigs.filter((cfg) => !cfg.is_maintenance).length} maintenance=${groupConfigs.filter((cfg) => cfg.is_maintenance).length}`);
  if (groupConfigs.length > 0) {
    console.log(`[check-cx][group-data] matchedConfigs=${groupConfigs.map((cfg) => `${cfg.groupName || "__ungrouped__"}::${cfg.name}`).join(" | ")}`);
  }
  ...
  const now = Date.now();
  const shouldBypassCache = refreshMode === "always" || options?.bypassCache === true || options?.forceRefreshNow === true;

  const loadData = async (): Promise<GroupDashboardData | null> => {
    if (options?.forceRefreshNow && activeConfigs.length > 0) {
      console.log(`[check-cx][group-data] forceRefreshNow target=${targetGroupName} activeConfigs=${activeConfigs.map((cfg) => `${cfg.groupName || "__ungrouped__"}::${cfg.name}`).join(" | ")}`);
      await runChecksForConfigs(activeConfigs, {notifyGroupName: targetGroupName});
    }
