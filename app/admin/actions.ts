...
import {triggerImmediatePoll} from "@/lib/core/poller";
...
import {DEFAULT_TELEGRAM_NOTIFICATION_SETTINGS, TELEGRAM_NOTIFICATION_SETTINGS_SINGLETON_KEY} from "@/lib/telegram-notification-settings";
import {buildDefaultGroupNameFromBaseUrl, buildOpenAiSelectedEndpoints, discoverProviderModels} from "@/lib/admin/model-discovery";
...
export async function refreshHomepageAction(formData: FormData): Promise<never> {
  return handleAction(formData, "refreshHomepage", "主页聚合缓存已刷新", async () => {
    invalidateOperationalCaches();
    const storage = await getControlPlaneStorage();
    await storage.checkConfigs.list();
    triggerImmediatePoll("admin-homepage-refresh");
  });
}

export async function discoverProviderModelsAction(formData: FormData): Promise<never> {
  return handleAction(formData, "discoverProviderModels", "模型拉取成功，请继续勾选要导入的模型", async () => {
    const type = getText(formData, "discover_type");
    const baseUrl = getText(formData, "discover_base_url");
    const apiKey = getOptionalText(formData, "discover_api_key");
    ensureProviderType(type);
    await discoverProviderModels({type, baseUrl, apiKey});
  });
}

export async function batchImportProviderModelsAction(formData: FormData): Promise<never> {
  return handleAction(formData, "batchImportProviderModels", "批量导入完成", async () => {
    const type = getText(formData, "discover_type");
    const baseUrl = getText(formData, "discover_base_url");
    ensureProviderType(type);
    const apiKey = getText(formData, "discover_api_key");
    if (!apiKey) throw new Error("批量导入时必须填写 API Key");

    const storage = await getControlPlaneStorage();
    const existing = await storage.checkConfigs.list();
    const selectedModels = formData.getAll("selected_models").map((item) => String(item)).map((item) => item.trim()).filter(Boolean);
    if (selectedModels.length === 0) throw new Error("请至少勾选一个模型");

    const includeChat = getBoolean(formData, "openai_endpoint_chat");
    const includeResponses = getBoolean(formData, "openai_endpoint_responses");
    const templateId = getOptionalText(formData, "template_id");
    const enabled = formData.get("enabled") !== null ? getBoolean(formData, "enabled") : true;
    const isMaintenance = getBoolean(formData, "is_maintenance");
    const defaultGroupName =
      getOptionalText(formData, "default_group_name_existing") ||
      getOptionalText(formData, "default_group_name_new") ||
      buildDefaultGroupNameFromBaseUrl(baseUrl);

    const endpoints = type === "openai"
      ? buildOpenAiSelectedEndpoints(baseUrl, includeChat, includeResponses)
      : [normalizeProviderEndpoint(type, baseUrl)];

    if (type === "openai" && endpoints.length === 0) {
      throw new Error("OpenAI 至少要勾选一个接口地址（chat/completions 或 responses）");
    }

    let created = 0, skipped = 0;
    for (const model of selectedModels) {
      const groupName = getOptionalText(formData, `group_for__${model}`) || defaultGroupName;
      if (groupName) {
        const group = await storage.groups.getByName(groupName);
        if (!group) {
          await storage.groups.upsert({group_name: groupName, website_url: null, tags: ""});
        }
      }
      for (const endpoint of endpoints) {
        const duplicate = existing.find((item) => item.type === type && item.model === model && item.endpoint === endpoint);
        if (duplicate) {
          skipped += 1;
          continue;
        }
        await storage.checkConfigs.upsert({
          name: `${groupName || type} / ${model} / ${endpoint.endsWith('/responses') ? 'responses' : endpoint.endsWith('/chat/completions') ? 'chat' : type}`,
          type,
          model,
          endpoint,
          api_key: apiKey,
          ...
          request_header: null,
          metadata: {import_source: 'batch-model-discovery', imported_at: new Date().toISOString(), base_url: baseUrl},
        });
        created += 1;
      }
    }
    triggerImmediatePoll(created > 0 ? "admin-config-batch-created" : "admin-config-batch-noop");
    if (created === 0 && skipped > 0) {
      throw new Error(`没有新增配置，${skipped} 条候选均已存在`);
    }

    const summary = skipped > 0 ? `批量导入完成：新增 ${created} 条，跳过 ${skipped} 条重复项` : `批量导入完成：新增 ${created} 条`;
    redirect(buildRedirectUrl(`/admin/configs?t=${Date.now()}`, "success", summary));
  });
}
...
export async function upsertConfigAction(formData: FormData): Promise<never> {
  return handleAction(formData, "upsertConfig", "检测配置已保存", async () => {
    ...
    const resolvedGroupName =
      getOptionalText(formData, "group_name_existing") ??
      getOptionalText(formData, "group_name_new") ??
      getOptionalText(formData, "group_name");
    ...
    if (payload.group_name) {
      const existingGroup = await storage.groups.getByName(payload.group_name);
      if (!existingGroup) {
        await storage.groups.upsert({group_name: payload.group_name, website_url: null, tags: ""});
      }
    }
    await storage.checkConfigs.upsert({id, ...payload});
    triggerImmediatePoll(id ? "admin-config-updated" : "admin-config-created");
  });
}
...
export async function deleteConfigAction(formData: FormData): Promise<never> {
  return handleAction(formData, "deleteConfig", "检测配置已删除", async () => {
    ...
    triggerImmediatePoll("admin-config-deleted");
  });
}
...
export async function deleteGroupAction(formData: FormData): Promise<never> {
  return handleAction(formData, "deleteGroup", "分组处理已完成", async () => {
    const id = getText(formData, "id");
    const mode = getOptionalText(formData, "delete_mode") ?? "migrate_to_ungrouped";
    if (!id) {
      throw new Error("缺少分组 ID");
    }

    const storage = await getControlPlaneStorage();
    const groups = await storage.groups.list();
    const target = groups.find((item) => item.id === id);
    if (!target) {
      throw new Error("未找到对应分组");
    }

    const configs = await storage.checkConfigs.list();
    const related = configs.filter((item) => (item.group_name ?? "") === target.group_name);

    if (mode === "cascade_configs") {
      for (const config of related) {
        await storage.checkConfigs.delete(config.id);
      }
    } else {
      for (const config of related) {
        await storage.checkConfigs.upsert({
          ...
          request_header: config.request_header ?? null,
...
export async function upsertTelegramNotificationSettingsAction(formData: FormData): Promise<never> {
  return handleAction(formData, "upsertTelegramNotificationSettings", "Telegram 通知设置已保存", async () => {
    const storage = await getControlPlaneStorage();
    const current = (await storage.telegramNotifications.getSingleton(TELEGRAM_NOTIFICATION_SETTINGS_SINGLETON_KEY)) ?? DEFAULT_TELEGRAM_NOTIFICATION_SETTINGS;
    const botToken = getText(formData, "telegram_bot_token") || current.bot_token;
    const chatId = getText(formData, "telegram_chat_id") || current.chat_id;
    await storage.telegramNotifications.upsert({
      singleton_key: TELEGRAM_NOTIFICATION_SETTINGS_SINGLETON_KEY,
      enabled: getBoolean(formData, "telegram_enabled"),
      bot_token: botToken,
      chat_id: chatId,
      notify_on_group_test: getBoolean(formData, "notify_on_group_test"),
      notify_on_full_test: getBoolean(formData, "notify_on_full_test"),
      notify_on_partial_refresh: getBoolean(formData, "notify_on_partial_refresh"),
      notify_on_auto_refresh: getBoolean(formData, "notify_on_auto_refresh"),
      only_on_failure_for_auto_refresh: getBoolean(formData, "only_on_failure_for_auto_refresh"),
    });
  });
}

export async function sendTelegramTestMessageAction(formData: FormData): Promise<never> {
  return handleAction(formData, "sendTelegramTestMessage", "Telegram 测试消息已发送", async () => {
    await sendTelegramTestMessage({
      botToken: getText(formData, "telegram_bot_token"),
      chatId: getText(formData, "telegram_chat_id"),
    });
  });
}
