...
export async function GET(_request: Request, context: RouteContext) {
  ...
  console.log(`[check-cx][group-api] request group=${decodedGroupName}`);
  ...
  console.log(`[check-cx][group-api] forceRefreshRequested=${String(forceRefreshRequested)} shouldForceRefresh=${String(shouldForceRefresh)} bypassCache=${String(shouldBypassCache)} trendPeriod=${trendPeriod ?? "default"}`);

  const data = await loadGroupDashboardData(decodedGroupName, {
    refreshMode: "never",
    trendPeriod,
    bypassCache: shouldBypassCache || shouldForceRefresh,
    forceRefreshNow: shouldForceRefresh,
